...
The first thing to do here is to prepare the sub-dataset that will be used to model the responses of the **efficiency variables**. To do so, the following steps have been performed:
* We selected a subset of variables for which we have hypotheses regarding tarping efficiency;

*eff_eradication* and *efficiency*. Of course, we also have to create the *efficiency* variable that does not exist yet. We will do it by averaging the three following variables: *eff_expansion*, *eff_dispersal*, and *eff_vigour*.
...
We already got a pretty good idea of the distribution of our variables and not all statistical methods involve a Normality assumption. Still, it's interesting and often useful to have a precise view of variables distributions, even for predictors and covariates. 
...
FAIRE efficiency§§§§§§§§§§§§§§§ + retirer les expés trop récentes!!
§§§§§§§§§§§§§§§§§§§§§§§
