...
erad %>% dplyr::select(-liner_geomem, -agri_geomem,-woven_geotex, -mulching_geotex, -pla_geotex, -other_unknown, -grammage, -age, -thickness, -pierced_tarpinstall, -add_control_type) %>%
  ...
  dplyr::filter(tarping_duration >= 2) -> erad
...
erad %>% dplyr::select(-xp_id, -goals, -preparation) -> num.erad
num.erad <-  dplyr::mutate_if(.tbl = num.erad, .predicate = is.factor, .funs = as.character)
num.erad <-  dplyr::mutate_if(.tbl = num.erad, .predicate = is.character, .funs = as.numeric) # I use this double conversion-trick because, strangely enough (but actually, there is a topic on SO about this), there is not R function to convert a factor into its numeric value! But we could create it like this:
# as.numeric.factor <- function(x) {as.numeric(levels(x))[x]}
# But it could not work in a pipe (because you have to specify `x`), so it's not that convenient.
...
mydata <- erad %>% dplyr::select(-xp_id, -goals, -eff_eradication, -high_eff, -preparation)

erad %>% dplyr::select(-xp_id, -goals, -eff_eradication, -high_eff, -preparation) %>%
  dplyr::mutate_if(is.factor, as.numeric.factor) -> mydata


predictors <- colnames(mydata)
# Tidying the data into the long-format for the plot (because we work here within the tidyverse!)
mydata %>% tidyr::pivot_longer(!efficiency, # It says that all columns of `mydata` need to be reshaped
                               # except "efficiency" (works as well with the form `-efficiency`, and we
                               # can place it somewhere else. Actually, it is the `cols =` argument!)
                               names_to = "Predictor", # Gives the name of the new grouping variable
                               values_to = "Value", # Gives the name of the variable that will be created
                               # from the data stored in the cell value
                               values_transform = list(Value = as.character)) -> mydata
ggplot2::ggplot(mydata, ggplot2::aes(x = Value, y = efficiency)) +
