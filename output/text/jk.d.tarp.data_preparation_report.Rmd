...
As multicollinearity is one of the biggest issues for inference modelling, we need to assess which explanatory variables and covariates are collinear and possibly remove them from our dataset. To do so, our intention was to compute *Variance Inflation Factors* (VIF) for each variable. However, as several variables are categorical and as we will not be working with models fitted with ordinary least squares, we will rather use *Generalized VIF* (Fox & Monette, 1992).

```{r erad init GVIF}
erad %>% dplyr::select(-xp_id, -goals, -eff_eradication, -high_eff) -> mydata

modvif <- stats::glm((efficiency/10)~latitude+longitude+elevation+freq_monitoring+slope+coarse_env+obstacles+flood+geomem+geotex+weedsp_geotex+maxveg+uprootexcav+stand_surface+fully_tarped+distance+tarping_duration+strips_overlap+tarpfix_multimethod+sedicover_height+trench_depth+plantation+repairs+add_control+degradation+pb_fixation+pb_durability, data = mydata, family = quasibinomial)
#summary(modvif)

knitr::kable(car::vif(modvif))
```

As one could expect, there is evidence for important **multicollinearity** among our 25 potential explanatory variables.  
After iteratively dropping strongly collinear variables and re-assessing GVIF values, we obtain a subset of potential explanatory variables with GVIF values < 2.2 (which is a very conservative threshold but could still be problematic if ecological signals are weak; cf. Zuur *et al.*, 2010). Naturally, we dropped in priority variables for which we had feeble hypotheses, but variables may not be entirely dropped (depending on which models they are included in) and they will not be forgotten in the discussion anyway.  
Here is **the list of the collinear variables** requiring careful consideration: *geotex*, *weedsp_geotex*, *latitude*, *longitude*, *coarse_env*, *pb_durability*, *degradation*, *flood*, *add_control*, *strips_overlap*, and *tarpfix_multimethod*.

```{r erad final GVIF}
modvif <- stats::glm((efficiency/10)~elevation+freq_monitoring+slope+obstacles+geomem+maxveg+uprootexcav+stand_surface+fully_tarped+distance+tarping_duration+sedicover_height+trench_depth+plantation+repairs+pb_fixation, data = mydata, family = quasibinomial)
#summary(modvif)

knitr::kable(car::vif(modvif))

erad %>% dplyr::select(-geotex, -weedsp_geotex, -latitude, -longitude, -coarse_env, -pb_durability, -degradation, -flood, -add_control, -strips_overlap, -tarpfix_multimethod) -> erad_light
