...
num.erad <- erad[, sapply(erad, is.numeric)]
nam <- names(num.erad)
num.erad <- as.matrix(num.erad)

par(mfrow= c (4,4), mar=c(3,2,0.5,1.5), cex.lab = 1.2, font.lab=2, bty = "n", fg = "gray35",
    col.axis = "gray35", col.lab = "gray20", cex = 0.6, tcl = -0.3,
    mgp = c(1.7, 0.6, 0.1), oma = c(1, 0, 1, 0), lab = c(5, 10, 7))
for (i in c(1:ncol(num.erad))) {
  plot(x = num.erad[,i], y = 1:length(num.erad[,i]), type = "p", xlab = nam[i], ylab = "", 
       col = "lightcoral", pch = 19, panel.first = {
         grid(col="lavender",nx = 5,ny = 9, lty = 6)
       }) }
