...
Finally, to further investigate potentially meaningful **interactions** among our variables, we used co-plots (i.e. conditioning plots) between some of our predictors, based on plausible hypotheses. However, to avoid overloading the present document, we only display some of the tested plots (do we? §§§§§§§§§§§§§§§§§§§§§§§

VERIFY before submitting, you piece of jelly!!!
...
### Three-way interaction between elevation, fully_tarped and maxveg
graphics::coplot(efficiency ~ elevation | fully_tarped * maxveg, data = erad, 
                 panel=function(x,y,...) { points(x,y, col = wesanderson::wes_palette(n=4, name="GrandBudapest1"), pch = 19); abline(line(y~x)) }, 
                 number = 3, overlap = 0.05)

### Three-way interaction between elevation, maxveg and uprootexcav
graphics::coplot(efficiency ~ elevation | maxveg * uprootexcav, data = erad, 
                 panel=function(x,y,...) { points(x,y, col = wesanderson::wes_palette(n=4, name="GrandBudapest1"), pch = 19); abline(line(y~x)) }, 
                 number = 3, overlap = 0.05)

# Interactions à tester:
elevation*fullyT*surface
elevation*(fullyT)*maxveg
elevation*maxveg*uprootexcav
elevation*time
elevation*plantation*followups

distance*surface
fully_tarped*surface
...
Our data are opportunistic blabla...
