...
erad %>% dplyr::select(-xp_id, -goals, -preparation) %>%
...
* *pb_fixation* seems to be strongly positively correlated with *stand_surface*.
...
As multicollinearity is one of the biggest issues for inference modelling, we need to assess which explanatory variables and covariates are collinear and remove them from our dataset. To do so, our intention was to compute *Variance Inflation Factors* (VIF) for each variable. However, as several variables are categorical and as we will not be working with models fitted with ordinary least squares, we will rather use *Generalized VIF* (Fox & Monette, 1992).

```{r erad init GVIF, fig.cap="**Table 1.** Generalised Variance Inflation Factors (GVIF) for the potential explanatory variables, prior to variable dropping"}
erad %>% dplyr::select(-xp_id, -goals, -eff_eradication, -high_eff, -preparation) -> mydata

modvif <- stats::glm((efficiency/10)~latitude+longitude+elevation+freq_monitoring+slope+coarse_env+obstacles+flood+geomem+geotex+weedsp_geotex+maxveg+stand_surface+fully_tarped+distance+tarping_duration+strips_overlap+tarpfix_multimethod+sedicover_height+trench_depth+plantation+repairs+add_control+degradation+pb_fixation+pb_durability, data = mydata, family = quasibinomial)
#summary(modvif)

knitr::kable(car::vif(modvif))
```

As one could expect, there is evidence for important **multicollinearity** among our 24 potential explanatory variables.  
After iteratively dropping strongly collinear variables and re-assessing GVIF values, we obtain a subset of potential explanatory variables with GVIF values < 2 (which is appropriate if we suspect weak ecological signals from the data; cf. Zuur *et al.*, 2010). Naturally, we dropped in priority variables for which we had feeble hypotheses, but dropped variables will not be forgotten in the discussion anyway.  
Here is **the list of the dropped variables:** *geotex*, *weedsp_geotex*, *latitude*, *longitude*, *coarse_env*, *pb_durability*, *pb_fixation*,*flood*, *add_control*, *strips_overlap*, and *tarpfix_multimethod*.

```{r erad final GVIF}
modvif <- stats::glm((efficiency/10)~elevation+freq_monitoring+slope+obstacles+geomem+maxveg+stand_surface+fully_tarped+distance+tarping_duration+sedicover_height+trench_depth+plantation+repairs+degradation, data = mydata, family = quasibinomial)
#summary(modvif)

knitr::kable(car::vif(modvif))

erad %>% dplyr::select(-xp_id, -goals, -eff_eradication, -high_eff, -preparation)
```
...
* The variables *efficiency*, *obstacles*, *flood*, *distance*, *tarping_duration*, *strips_overlap*, and *trench_depth* may require mild transformations to be normalized but do not seem to contain too extreme values.
...
* The variables *longitude*, and *stand_surface* contain problematic patterns/outliers. As longitude is supposed to be used as a ***random effect*** in our models, I am unsure about the consequences of extreme values or bimodal distributions (but I guess it is not good). Still, longitude is unlikely to effectively affect tarping success, so we can consider dropping this variable. 


```{r erad simu.distrib, include=FALSE, eval=FALSE}
simu.var <- dplyr::select(.data = erad, elevation, freq_monitoring, stand_surface, distance, strips_overlap, sedicover_height, trench_depth, tarping_duration)
...
After analysing random samples drawn from log-normal distributions (based on the parameters of our potentially problematic variables), we have good reason to believe that only *longitude* and *stand_surface* should be problematic. Consequently, we choose to delete the outlier spotted for *stand_surface* (i.e. the *stand_surface* of ca. 5000 m^2^) and drop the *longitude* variable.   
...
The next step in our data exploration is to plot the response variables against each predictors and covariates in order to assess the **linearity** assumption and identify potential **interactions** and **multivariate outliers**. It would be interesting to plot all variables against each other, but we have too many potential predictors and covariates for that.
...
# erad %>% dplyr::select(-xp_id, -goals, -preparation) %>% GGally::ggpairs()
...
erad %>% dplyr::select(-xp_id, -goals, -eff_eradication, -high_eff, -preparation) %>%
  ...
  ggplot2::facet_wrap(~Predictor, scales = "free", strip.position = "bottom") +
...
Include weights??? (e.g. tarping duration?) efficiency with quasi-binomial GLM?
