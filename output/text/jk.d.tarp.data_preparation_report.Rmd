...
### - Multivariate synthesis of contextual variables
\
...
As one could expect, there is evidence for **multicollinearity** among our 26 potential explanatory variables. However, strong collinearity among variables is only a problem if these variables are included together in the same model, hence we will have to reevaluate collinearity more precisely when actual working models will be built. Here, we simply identified which variables could be generally too problematic across likely models and, possibly, dropped them (we tried to lower GVIF values as much as possible, considering that, even GVIF of 2 could still be problematic if ecological signals are weak; cf. Zuur *et al.*, 2010):

* The *geomem* and *geotex* (and *weedsp_geotex*) variables are, without a surprise, problematic as they contain the same information. Therefore, we decided to only keep *geomem*.
* The *plantation* variable also seems highly problematic. Yet, as we had a strong interest in that variable, we chose to drop it **only temporarily**, only to detect other problematic variables. A particular attention to multicollinearity should be maintained when this variable is included within models.
* Since our dataset contained sites on both sides of the Atlantic and since we did not have strong hypotheses regarding the effects of *longitude* (because our sites were located well within the longitudinal range of knotweeds), we decided to drop it.
* Since *degradation* contained most of the information contained in *pb_fixation* and *pb_durability*, we decided to drop it to reduce the number of variables. 
...
```{r erad coplot 3, fig.align='center', fig.cap="**Figure 2.8c.** Coplot of the *efficiency score* displaying a two-way interaction between *followups* and *pb_fixation*. There is a 5% overlap between each group, explaining why there are more than *n* = 85 points. Colours are meaningless and are just here for aesthetical reasons", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
graphics::coplot(efficiency ~ followups | pb_fixation, data = erad, 
...
```
```{r erad coplot 4, fig.align='center', fig.cap="**Figure 2.8d.** Coplot of the *efficiency score* displaying a two-way interaction between *followups* and *pb_durability*. There is a 5% overlap between each group, explaining why there are more than *n* = 85 points. Colours are meaningless and are just here for aesthetical reasons", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
graphics::coplot(efficiency ~ followups | pb_durability, data = erad, 
                 panel=function(x,y,...) { points(x,y, col = wesanderson::wes_palette(n=4, name="Royal2"), pch = 19); abline(line(y~x)) }, 
                 number = 2, overlap = 0.05, rows = 1)
```

Quite logically, these two coplots seem to confirm that follow-up operations increase tarping efficiency especially when there are some problems regarding fabrics fixation or durability. The pattern is particularly clear for fixation problems (Figure 2.8c).
...
```{r redges followups, warning=FALSE, fig.align='center', fig.cap="**Figure 3.1 bis.** Correlation plot of the normed-PCA on follow-up variables for the *tarping efficiency* dataset"}
...
```{r redges corrplot, fig.align='center', fig.cap="**Figure 3.2.** Correlation matrix displaying *Spearman*'s $\\rho$ for the numeric variables and ordered factors of the *regrowth at the edges of the tarped area* dataset. Only significant correlations are displayed (with $\\alpha$ = 0.05)", warning=FALSE}
rm(res.pca, xxx, ttt)
...
As one could expect, there is evidence for **multicollinearity** among our 25 potential explanatory variables. However, strong collinearity among variables is only a problem if these variables are included together in the same model, hence we will have to reevaluate collinearity more precisely when actual working models will be built. Here, we simply identified which variables could be generally too problematic across likely models and, possibly, dropped them (we tried to lower GVIF values as much as possible, considering that, even GVIF of 2 could still be problematic if ecological signals are weak; cf. Zuur *et al.*, 2010):

* The *geomem* and *geotex* variables are, without a surprise, problematic as they contain the same information. Therefore, we decided to only keep *geomem*.
* The *plantation* variable also seems highly problematic. Yet, as we had a strong interest in that variable, we chose to drop it **only temporarily**, only to detect other problematic variables. A particular attention to multicollinearity should be maintained when this variable will be included within models.
* Since our dataset contained sites on both sides of the Atlantic and since we did not have strong hypotheses regarding the effects of *longitude* (because our sites were located well within the longitudinal range of knotweeds), we decided to drop it.
* Since *degradation* contained most of the information contained in *pb_fixation* and *pb_durability*, we decided to drop it to reduce the number of variables. 
* As *coarse_env* is strongly positively correlated with *obstacles* and *slope*, we decided to drop it in order to reduce multicollinearity as well as the number of variables. 
...
```{r redges boxplot outliers, fig.align='center', fig.cap="**Figure 3.3.** Boxplots for all quantitative variables of the *regrowth at the edges of the tarped area* dataset", warning=FALSE}
jk.dusz.tarping::uni.boxplots(dataset = redges)
```

```{r redges dotplot, fig.align='center', fig.cap="**Figure 3.4.** Cleveland dotplots for all quantitative variables of the *regrowth at the edges of the tarped area* dataset"}
...
After analysing random samples drawn from log-normal distributions (based on the parameters of our potentially problematic variables), we have good reason to believe that only *stand_surface* should be problematic. Consequently, we chose to delete the spotted outlier within *stand_surface* (i.e. the *stand_surface* of ca. 5000 m^2^).    
The other variables do not have distribution that a transformation could not fix, except perhaps *latitude* (we will keep an eye on the influence of its single extreme value). We will see later on whether we actually need such transformation (e.g log), and if we actually use this variable in modelling. 
...
```{r redges histo, fig.align='center', fig.cap="**Figure 3.5.** Histograms for all quantitative variables of the *regrowth at the edges of the tarped area* dataset"}
...
```{r redges bivariate plot, fig.align='center', fig.cap="**Figure 3.6.** Multivariate representation of the *lreg_edges* against each potential explanatory variable", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
### To plot a response variable against all predictors/covariates, we will have to do use a for loop or the facet_wrap function of ggplot (or equivalent).
...
As our response variable (i.e. *lreg_edges*) is binomial, **heteroscedasticity** is not a problem as variance is assumed to be heterogeneous in logistic regression models.
\
\
...
```{r redges coplot 1, fig.align='center', fig.cap="**Figure 3.7a.** Conditioning spineplot of *lreg_edges* displaying a three-way interaction between *distance*, *stand_surface* (in log-scale) and *trench_depth*. Within each panel, the proportion of operations that found knotweed regrowths at the edge of the tarped area (dark orange) and the proportion of those that did not (light orange) is displayed, while the left-hand rectangles display these proportions for "low" tarping distances (below average) and the right-hand rectangles display them for "high" tarping distances (above average)", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}

par(.pardefault) # To restore defaults graphical parameters

redges %>% dplyr::mutate(distance = ifelse(distance <= mean(distance), "Low", "High")) %>%
  dplyr::mutate(distance = factor(distance, levels=c("Low", "High"))) -> rrr
...
```{r redges coplot 2, fig.align='center', fig.cap="**Figure 3.7b.** Conditioning spineplot of *lreg_edges* displaying a two-way interaction between *slope* and *obstacles*. Within each panel, the proportion of operations that found knotweed regrowths at the edge of the tarped area (dark orange) and the proportion of those that did not (light orange) is displayed, while the left-hand rectangles display these proportions for "low" slopes (below average) and the right-hand rectangles display them for "high" slopes (above average)", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}

redges %>% dplyr::mutate(slope = ifelse(slope <= mean(slope), "Low", "High")) %>%
  dplyr::mutate(slope = factor(slope, levels=c("Low", "High"))) -> rrr
...
```{r redges coplot 2, fig.align='center', fig.cap="**Figure 3.7c.** Conditioning spineplot of *lreg_edges* displaying a three-way interaction between *obstacles*, *geomem* and *tarpfix_multimethod*. Within each panel, the proportion of operations that found knotweed regrowths at the edge of the tarped area (dark orange) and the proportion of those that did not (light orange) is displayed, while the left-hand rectangles display these proportions for "low" obstacle proportions (below average) and the right-hand rectangles display them for "high" obstacle proportions (above average)", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
...
```{r redges coplot 2, fig.align='center', fig.cap="**Figure 3.7d.** Conditioning spineplot of *lreg_edges* displaying a two-way interaction between *obstacles* and *distance*. Within each panel, the proportion of operations that found knotweed regrowths at the edge of the tarped area (dark orange) and the proportion of those that did not (light orange) is displayed, while the left-hand rectangles display these proportions for "low" obstacle proportions (below average) and the right-hand rectangles display them for "high" obstacle proportions (above average)", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
...
# - Exploration of the sub-dataset related to the response variable indicating knotweed regrowths within the tarped area (i.e. *lreg_tarpedarea*)
\
...
```{r rtarped env PCAs, warning=FALSE, fig.align='center', fig.cap="**Figure 4.1.** Correlation plot of the normed-PCA on environmental variables for the *regrowth within the tarped area* dataset"}
...
```{r rtarped followups, warning=FALSE, fig.align='center', fig.cap="**Figure 4.1 bis.** Correlation plot of the normed-PCA on follow-up variables for the *tarping efficiency* dataset"}
...
```{r rtarped corrplot, fig.align='center', fig.cap="**Figure 4.2.** Correlation matrix displaying *Spearman*'s $\\rho$ for the numeric variables and ordered factors of the *regrowth within the tarped area* dataset. Only significant correlations are displayed (with $\\alpha$ = 0.05)", warning=FALSE}
rm(res.pca, xxx, ttt)
...
As one could expect, there is evidence for **multicollinearity** among our 31 potential explanatory variables. However, strong collinearity among variables is only a problem if these variables are included together in the same model, hence we will have to reevaluate collinearity more precisely when actual working models will be built. Here, we simply identified which variables could be generally too problematic across likely models and, possibly, dropped them (we tried to lower GVIF values as much as possible, considering that, even GVIF of 2 could still be problematic if ecological signals are weak; cf. Zuur *et al.*, 2010):

* The *geomem*, *geotex*, and *woven_geotex* variables are, without a surprise, problematic as they contain the same information. Therefore, we decided to drop *geotex*.
* The *plantation* variable also seems highly problematic. Yet, as we had a strong interest in that variable, we chose to drop it **only temporarily**, only to detect other problematic variables. A particular attention to multicollinearity should be maintained when this variable is included within models.
* Since our dataset contained sites on both sides of the Atlantic and since we did not have strong hypotheses regarding the effects of *longitude* (because our sites were located well within the longitudinal range of knotweeds), we decided to drop it.
* As much of the information contained in *obstacles* is shared with *pierced_tarpinstall* and *coarse_env*, we decided to drop it in order to reduce multicollinearity as well as the number of variables. 
...
```{r rtarped boxplot outliers, fig.align='center', fig.cap="**Figure 4.3.** Boxplots for all quantitative variables of the *regrowth within the tarped area* dataset", warning=FALSE}
jk.dusz.tarping::uni.boxplots(dataset = rtarped)
```

```{r rtarped dotplot, fig.align='center', fig.cap="**Figure 4.4.** Cleveland dotplots for all quantitative variables of the *regrowth within the tarped area* dataset"}
...
```{r rtarped histo, fig.align='center', fig.cap="**Figure 4.5.** Histograms for all quantitative variables of the *regrowth within the tarped area* dataset"}
...
```{r rtarped bivariate plot, fig.align='center', fig.cap="**Figure 4.6.** Multivariate representation of *lreg_tarpedarea* against each potential explanatory variable", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
### To plot a response variable against all predictors/covariates, we will have to do use a for loop or the facet_wrap function of ggplot (or equivalent).
...
As our response variable (i.e. *lreg_tarpedarea*) is binomial, **heteroscedasticity** is not a problem as variance is assumed to be heterogeneous in logistic regression models.
\
\
...
```{r roverlaps corrplot, fig.align='center', fig.cap="**Figure 5.2.** Correlation matrix displaying *Spearman*'s $\\rho$ for the numeric variables and ordered factors of the *regrowth at strips overlap* dataset. Only significant correlations are displayed (with $\\alpha$ = 0.05)", warning=FALSE}
rm(res.pca, xxx, ttt)
...
As one could expect, there is evidence for **multicollinearity** among our 27 potential explanatory variables. However, strong collinearity among variables is only a problem if these variables are included together in the same model, hence we will have to reevaluate collinearity more precisely when actual working models will be built. Here, we simply identified which variables could be generally too problematic across likely models and, possibly, dropped them (we tried to lower GVIF values as much as possible, considering that, even GVIF of 2 could still be problematic if ecological signals are weak; cf. Zuur *et al.*, 2010):

* As *levelling* is highly correlated with *uprootexcav* and seems to be the most problematic variable in terms of multicollinearity, we decided to drop *uprootexcav*.
* As we did not have a strong hypothesis regarding the effect of *floods* on the probability of finding regrowths at strips overlap, we decided to drop the *flood* variable and, as much of the information contained in *obstacles* is also contained in *coarse_env*, we decide to drop it as well.
* Finally, we decided to drop *maxveg* for similar reasons.
...
```{r roverlaps boxplot outliers, fig.align='center', fig.cap="**Figure 5.3.** Boxplots for all quantitative variables of the *regrowth at strips overlap* dataset", warning=FALSE}
jk.dusz.tarping::uni.boxplots(dataset = roverlaps)
```

```{r roverlaps dotplot, fig.align='center', fig.cap="**Figure 5.4.** Cleveland dotplots for all quantitative variables of the *regrowth at strips overlap* dataset"}
...
```{r roverlaps histo, fig.align='center', fig.cap="**Figure 5.5.** Histograms for all quantitative variables of the *regrowth at strips overlap* dataset"}
...
```{r roverlaps bivariate plot, fig.align='center', fig.cap="**Figure 5.6.** Multivariate representation of *reg_stripsoverlap* against each potential explanatory variable", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
### To plot a response variable against all predictors/covariates, we will have to do use a for loop or the facet_wrap function of ggplot (or equivalent).
...
As our response variable (i.e. *reg_stripsoverlap*) is binomial, **heteroscedasticity** is not a problem as variance is assumed to be heterogeneous in logistic regression models.
\
\
...
1) Set drake + ask questions SO/CV !!! + TESTER si stand_surface change de coef quand /10 (en dizaine de m2)
...
Harrell, F., 2015. Regression Modeling Strategies. With Applications to Linear Models, Logistic and Ordinal Regression, and Survival Analysis. Springer Nature Switzerland AG: Cham (582 p.).
