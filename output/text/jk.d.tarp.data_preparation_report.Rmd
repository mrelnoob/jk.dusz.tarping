...
* Since our dataset contained sites on both sides of the Atlantic and since we did not have strong hypotheses regarding the effects of *longitude* (because our sites are located well within the longitudinal range of knotweeds), we decided to drop it.
...
Finally, to further investigate potentially meaningful **interactions** among our variables, we used co-plots (i.e. conditioning plots) between some of our predictors and *lreg_tarpedarea*, based on plausible hypotheses. 
However, as *lreg_tarpedarea* is a binary factor, the use of coplots was more complicated than for a numeric variable. Visualizing categorical variables as a function of other variables, especially quantitative ones, is not ideal in a regular conditioning plot as points will tend to be stacked on each other and as logistic curves should be fitted instead of regression lines, which is quite tedious to automatise in R. Consequently, we chose to draw *spineplots within the coplot panels* (**conditioning spineplots**) by transforming quantitative variables into two-level factors ("low" and "high" - based on whether observations had values below or above average, meaning that the number of observations is not distributed equitably between the two categories). The plots created could be viewed as some sort of *graphical contingency tables*. Unfortunately, combining spineplots with coplots creates a graphic bug in the drawing of the axes (linked to the coplot function internals) that we could not fix yet. We apologize for that.

To avoid overloading the present document, only the most informative plots are displayed. 

```{r rtarped coplot 1, fig.align='center', fig.cap="**Figure 4.7a.** Conditioning spineplot of *lreg_tarpedarea* displaying a three-way interaction between *distance*, *stand_surface* (in log-scale) and *trench_depth*. Within each panel, the proportion of operations that found knotweed regrowths within the tarped area (dark blue) and the proportion of those that did not (light blue) is displayed, while the left-hand rectangles display these proportions for "low" tarping distances (below average) and the right-hand rectangles display them for "high" tarping distances (above average)", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}

par(.pardefault) # To restore defaults graphical parameters

rtarped %>% dplyr::mutate(stand_surface = ifelse(stand_surface <= mean(stand_surface), "Low", "High")) %>%
  dplyr::mutate(stand_surface = factor(stand_surface, levels=c("Low", "High"))) -> rrr

pan.fun = function(x, y, ...){
    usr <- par("usr"); on.exit(par(usr)) # Frankly, I don't understand what these two lines do (only new = TRUE seems to be doing something), and the spineplot messes with the axes of the coplots below.
    par(usr = c(usr[1:2], 0, 1.5), new=T)
    spineplot(as.factor(x), as.factor(y), main="", xlab="", ylab="", axes=F, col = c("#33CCCC","#CCFFFF"))
}

### Three-way interaction between stand_surface, stand_surface and trench_depth:
graphics::coplot(lreg_tarpedarea ~ stand_surface | flood, data = rrr, 
                 panel = pan.fun, number = 3, overlap = 0.01, rows = 1)

coarse_env*woven_geotex*followups
coarse_env*geomem
coarse_env*stand_surface
coarse_env*levelling*woven/geomem
coarse_env*stripsoverlap_ok
coarse_env*tarpfix_multimethod
coarse_env*plantation
coarse_env*followups
coarse_env*tarping_duration

stand_surface*geomem/woven
stand_surface*levelling*flood
stand_surface*stripsoverlap_ok
stand_surface*tarpfix
stand_surface*piercedinstall*followups
stand_surface*plantation
stand_surface*followups
stand_surface*duration

woven/geomem*tarpfix_pierced*levelling/followups
woven/geomem*duration*???

levelling*flood?
levelling*duration
levelling*tarpfix_pirced

stripsoverlap_ok*flood
stripsoverlap_ok*woven
stripsoverlap_ok*levelling
stripsoverlap_ok*tarpfix_multimethod
stripsoverlap_ok*piercedinstal
stripsoverlap_ok*followups
stripsoverlap_ok*plantation*followups/tarpfixmulti
```

This conditioning spineplot is quite informative because
