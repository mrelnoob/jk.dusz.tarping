...
# - Exploration of the sub-dataset related to the response variables indicating knotweed regrowths at the edge of the tarped area (i.e. *lreg_edges*)
\
...
However, as *lreg_edges* is a binary factor, the use of coplots got more complicated. Visualizing categorical variables as a function of other variables, especially quantitative ones, is not ideal in a regular conditioning plot as points will tend to be stacked on each other and as logistic curves should be fitted instead of regression lines, which is quite tedious to automatise in R. Consequently, we chose to draw *spineplots within the coplot panels* (**conditioning spineplots**) by transforming quantitative variables into two-level factors ("low" and "high" - based on whether observations had values below or above average, meaning that the number of observations is not distributed equitably between the two categories). The plots created could be viewed as some sort of *graphical contingency tables*. Unfortunately, combining spineplots with coplots creates a graphic bug in the drawing of the axes (linked to the coplot function internals) that we could not fix yet. We apologize for that.
...
This conditioning spineplot is quite informative because it shows a changing pattern as we move from bottom to top, but globally not from left to right. It could be the sign of an existing interaction between *distance* and *trench depth*, but not with *stand surface*: with increasing trench depth, the proportion of tarping operations that found no knotweed regrowths at the edges (i.e. light orange rectangles) when tarping distances were above average (i.e. right-hand rectangles within each of the 9 panels) increases, regardless of the size of the knotweed stands. Actually, it is hard to say if stand surface does not play any role here but it appears that there is no unequivocal trend.


```{r redges coplot 2, fig.align='center', fig.cap="**Figure 3.7b.** Conditioning spineplot of *lreg_edges* displaying a three-way interaction between *distance*, *stand_surface* (in log-scale) and *trench_depth*. Within each panel, the proportion of operations that found knotweed regrowths at the edge of the tarped area (dark orange) and the proportion of those that did not (light orange) is displayed, while the left-hand rectangles display these proportions for "low" tarping distances (below average) and the right-hand rectangles display them for "high" tarping distances (above average)", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
...
### Three-way interaction between obstacles, stand_surface and trench_depth:
graphics::coplot(lreg_edges ~ obstacles | pb_fixation, data = rrr, 
                 panel = pan.fun, number = 3, overlap = 0.01, rows = 1)


slope*flood*obstacles
slope*distance
slope*tarpfix_multimethod
flood*tarpfix_multimethod
slope*trench_depth
flood*trench_depth
slope*plantation
slope*followups
slope*pb_fixation
slope*tarping_duration
# OR FLOOOOOOOD????

obstacles*stand_surface # Nope!
obstacles*geomem # WHY NOT§§§§§§§§§§§§
obstacles*uprootexcav # Mouais
obstacles*distance # Ok!
obstacles*tarpfix_multimethod #Mouais
obstacles*trench_depth # Nope!
obstacles*plantation # Nope!
obstacles*followups # Nope!
obstacles*pb_fixation # Nope!

distance*stand_surface*trench_depth # Ok!
distance*stand_surface*maxveg/uproot # Nope!
distance*trench_depth # Nope!
distance*plantation # Nope!
distance*duration # Nope!
distance*pb_fixation # Nope!
distance*tarpfix_multimethod # Nope!
```
...
graphics::coplot(efficiency ~ followups | pb_fixation, data = redges, 
                 panel=function(x,y,...) { points(x,y, col = wesanderson::wes_palette(n=4, name="Royal2"), pch = 19); abline(line(y~x)) }, 
                 number = 2, overlap = 0.05, rows = 1)
```
```{r redges coplot 4, fig.align='center', fig.cap="**Figure 2.8d.** Coplot of the *efficiency score* displaying a two-way interaction between *followups* and *pb_durability*. There is a 5% overlap between each group, explaining why there are more than *n* = 85 points. Colours are meaningless and are just here for aesthetical reasons", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
graphics::coplot(efficiency ~ followups | pb_durability, data = redges, 
                 panel=function(x,y,...) { points(x,y, col = wesanderson::wes_palette(n=4, name="Royal2"), pch = 19); abline(line(y~x)) }, 
                 number = 2, overlap = 0.05, rows = 1)
```
...
# - Exploration of the sub-dataset related to the response variables indicating knotweed regrowths within the tarped area (i.e. *lreg_tarpedarea*)
\
