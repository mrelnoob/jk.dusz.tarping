...
graphics::coplot(lreg_edges ~ elevation | log(stand_surface), data = redges, 
                 panel=function(x,y,...) { 
                   fit <- glm(y~x, family = binomial);
                   newdat <- data.frame(x=seq(min(redges$elevation), max(redges$elevation),len=100));
                   newdat$y = predict(fit, newdata=newdat, type="response");
                   points(x,y, col = wesanderson::wes_palette(n=4, name="GrandBudapest1"), pch = 19);
                   lines(y ~ x, newdat, col="green4", lwd=2)
                   }, 
                 number = 3, overlap = 0.05, rows = 1)
